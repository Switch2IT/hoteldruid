...
*/
        ...
        if ($importa_tariffa and $priv_mod_tariffe != "n") {
            ...
            if ($errore != "SI") {
                ...
                if ($dati_tariffe[$tariffa_a]['importa_prezzi'] or $dati_tariffe[$tariffa_da]['importa_prezzi']) $errore = "SI";
                if ($dati_tariffe[$tariffa_a]['esporta_prezzi'] and $tipo_importa == "sempre") {
                    $errore = "SI";
                    echo mex("La tariffa", $pag) . " <b class=\"colred\">$num_tariffa_a</b> " . mex("esporta i prezzi ad altre tariffe, non può importare i prezzi in modo permanente", $pag) . ".<br>";
                    $mostra_ok = 1;
                    $ancora = "imp_pre";
                } # fine if ($dati_tariffe[$tariffa_a]['esporta_prezzi'] and $tipo_importa == "sempre")
                if ($errore != "SI") {
                    $aggiorna_tariffe_interconn = "SI";
                    $prezzi_da = esegui_query("select idperiodi,$tariffa_da,$tariffa_da" . "p from $tableperiodi where $tariffa_da is not NULL or $tariffa_da" . "p is not NULL");
                    ...
                    if ($tipo_importa == "sempre") {
                        ...
                        esegui_query("update $tablenometariffe set $tariffa_a = '$val_impor' where idntariffe = '6' ");
        ...
        if ($tar_importa_canc and $priv_mod_tariffe != "n") {
            $errore = 0;
            if (controlla_num_pos($tar_importa_canc) != "SI") $errore = 1;
            if ($attiva_tariffe_consentite != "n" and $tariffe_consentite_vett["tariffa$tar_importa_canc"] != "SI") $errore = 1;
            if (!$errore) {
                $tabelle_lock = array($tablenometariffe);
                $tabelle_lock = lock_tabelle($tabelle_lock);
                $dati_tariffe = dati_tariffe($tablenometariffe);
                if ($dati_tariffe["tariffa$tar_importa_canc"]['importa_prezzi']) {
                    $mostra_form_iniziale = "NO";
                    esegui_query("update $tablenometariffe set tariffa$tar_importa_canc = NULL where idntariffe = '6' ");
                    echo mex("L'importatazione dei prezzi è stata cancellata", $pag) . ".<br>";
                    $mostra_ok = 1;
                    $ancora = "imp_pre";
                } # fine if ($dati_tariffe["tariffa$tar_importa_canc"]['importa_prezzi'])
                unlock_tabelle($tabelle_lock);
            } # fine if (!$errore)
        } # fine if ($tar_importa_canc and $priv_mod_tariffe != "n")
...
# Inserimento rapido costi aggiuntivi
            if ($ins_rapido_costo and $priv_ins_costi_agg != "n") {
                ...
                if ($tar_err or $vmm_err or $nom_err or $tas_err or $lim_err or $val_err or $agm_err or $adg_err or $apc_err or $per_err) $inseriscicosti = "";
...
<label><input type=\"radio\" id=\"tsb_f\" name=\"tipo_val_rapido\" value=\"f\"$checked> " . mex("fisso di", $pag) . "
 ...
 onclick=\"document.getElementById('tsb_f').checked='1'\"><label for=\"tsb_f\">$Euro";
                        if ($tipocostoagg == "off_spec") echo " " . mex("al giorno", $pag);
                        echo "</label></td></tr>";
...
<input type=\"hidden\" name=\"idapp_incomp_$num1\" value=\"$idappartamenti\">";
                    ...
                    if ($tipocostoagg == "letto_agg") {
                        ...
                        echo mex("Limitarne il numero che è possibile avere contemporaneamente in uno stesso periodo", $pag) . "?</td>
...
# Inserisco i costi aggiuntivi
            if ($inseriscicosti and $priv_ins_costi_agg != "n") {
                ...
                if ($passo > 1) {
                    $mostra_form_iniziale = "NO";
                    if ($id_utente != 1 and $passo >= 12) $tabelle_lock = array($tablenometariffe, $tableprivilegi);
                    else $tabelle_lock = array($tablenometariffe);
...
<br><hr style=\"width: 95%\">";
                ...
                $dati_tariffe = dati_tariffe($tablenometariffe);
                if ($id_utente == 1) {
                    echo "<table cellpadding=\"0\" cellspacing=\"0\"><tr><td style=\"height: 6px;\"></td></tr><tr><td>
                ...
                if ($priv_mod_tariffe != "n") {
                    ...
                    for ($num1 = 1; $num1 <= $dati_tariffe['num']; $num1++) {
                        if ($attiva_tariffe_consentite == "n" or $tariffe_consentite_vett[$num1] == "SI") {
                            ...
                            if (!$dati_tariffe[$tariffa]['importa_prezzi']) $lista_opt_tariffe_cambia .= "<option value=\"$tariffa\">$nometariffa_vedi</option>";
...
" . mex("i prezzi della", $pag) . " ";
                        $select_nomi_tariffe = "";
                        echo "<select name=\"tariffa_a\">
$lista_opt_tariffe_cambia
</select> " . mex("dalla", $pag) . " <select name=\"tariffa_da\">
$lista_opt_tariffe_cambia
</select> " . mex("aggiungendo", $pag) . " <span style=\"white-space: nowrap;\">
<input type=\"text\" name=\"importa_percent\" size=\"3\" value=\"0\"><select name=\"tipo_percent\" id=\"tip_per\" onchange=\"att_dis_arrotond()\"> 
<option value=\"perc\">%</option>
<option value=\"euro_g\">$Euro " . mex("$parola_alla $parola_settimana", $pag) . "</option>";
                        if ($tipo_periodi == "g") echo "<option value=\"euro_s\">$Euro " . mex("alla settimana", $pag) . "</option>";
                        echo "</select></span> (" . mex("arrotondato a", $pag) . "
<input type=\"text\" name=\"importa_arrotond\" id=\"imp_arr\" value=\"$arrotond_predef\" size=\"4\">)
 <select name=\"parte_prezzo\">
<option value=\"f\">" . mex("al prezzo fisso", $pag) . "</option>
...
<button class=\"xpri\" type=\"submit\"><div>" . mex("importa", $pag) . "</div></button>
                        ...
                        for ($num1 = 1; $num1 <= $dati_tariffe['num']; $num1++) {
                            if ($attiva_tariffe_consentite == "n" or $tariffe_consentite_vett[$num1] == "SI") {
                                $tariffa = "tariffa" . $num1;
                                if ($dati_tariffe[$tariffa]['importa_prezzi']) {
                                    echo "<form accept-charset=\"utf-8\" method=\"post\" action=\"creaprezzi.php\"><div>
<input type=\"hidden\" name=\"anno\" value=\"$anno\">
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input type=\"hidden\" name=\"tar_importa_canc\" value=\"$num1\">
" . mex("Importa sempre", $pag) . " " . mex("i prezzi della", $pag) . " <b>" . mex("tariffa", $pag) . "$num1";
                                    if ($dati_tariffe[$tariffa]['nome']) echo " (" . $dati_tariffe[$tariffa]['nome'] . ")";
                                    echo "</b> " . mex("dalla", $pag) . " <em>" . mex("tariffa", $pag) . $dati_tariffe[$tariffa]['importa_prezzi'];
                                    if ($dati_tariffe['tariffa' . $dati_tariffe[$tariffa]['importa_prezzi']]['nome']) echo " (" . $dati_tariffe['tariffa' . $dati_tariffe[$tariffa]['importa_prezzi']]['nome'] . ")";
                                    echo "</em>";
                                    if ($dati_tariffe[$tariffa]['val_importa']) {
                                        echo " " . mex("aggiungendo", $pag) . " " . $dati_tariffe[$tariffa]['val_importa'];
                                        if ($dati_tariffe[$tariffa]['tipo_importa'] == "p") echo "% (" . mex("arrotondato a", $pag) . " " . $dati_tariffe[$tariffa]['arrotond_importa'] . " $Euro)";
                                        if ($dati_tariffe[$tariffa]['tipo_importa'] == "g") echo " $Euro " . mex("$parola_alla $parola_settimana", $pag) . "";
                                        if ($dati_tariffe[$tariffa]['tipo_importa'] == "s") echo " $Euro " . mex("alla settimana", $pag) . "";
                                        if ($dati_tariffe[$tariffa]['parte_prezzo'] == "f") echo " " . mex("al prezzo fisso", $pag) . "";
