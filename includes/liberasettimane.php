...
# di appartamenti richiesti, rimuovendo anche qui gli appartamenti con prenotazioni assegnabili solo nei gruppi. 
    if ($app_richiesti_vett[',vicini,'] == "SI") {
        ...
        if (!$app_richiesti_vett[',succ_non_vicino,'][$app_richiesti_vett['id']]) {
            $app_controllati = ",";
            for ($num1 = 0; $num1 < $dati_app['totapp']; $num1++) if ($dati_app['vicini'][$numapp] == $numapp and (!$app_richiesti or $app_richiesti[$numapp] == "SI")) $app_non_liberabili .= "$numapp,";
            if ($app_richiesti_vett['id'] == 1 or $app_richiesti_vett[',succ_non_vicino,'][($app_richiesti_vett['id'] - 1)]) {
                ...
                for ($num1 = 0; $num1 < $dati_app['totapp']; $num1++) {
                    ...
                    if ((!$app_richiesti or $app_richiesti[$numapp] == "SI") and !strstr($app_controllati, ",$numapp,") and !strstr($app_non_liberabili, ",$numapp,")) {
                    ...
                    } # fine if ((!$app_richiesti or $app_richiesti[$numapp] == "SI") and !strstr($app_controllati,",$numapp,"))
                } # fine for $num1
                ...
                elseif ($num_vicini_tot > $num_vicini and $app_gruppi_disp) {
                    for ($num1 = 0; $num1 < $dati_app['totapp']; $num1++) {
                        ...
                        if (@is_array($prenota_ricontrolla_tot[$numapp])) {
                            while (list($num_pp, $vett_assegnabili) = each($prenota_ricontrolla_tot[$numapp])) {
                                $num_assegnabili = count($vett_assegnabili);
                            ...
                            } # fine while (list($num_pp,$vett_assegnabili) = each($prenota_ricontrolla_tot[$numapp]))
