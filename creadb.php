...
if (C_CREA_ULTIMO_ACCESSO != "SI" or !@is_file(C_DATI_PATH . "/ultimo_accesso")) {
    ...
    if ($id_utente and $id_utente == 1) {
        ...
        if ($creabase and !@is_file(C_DATI_PATH . "/dati_connessione.php")) {
            ...
            if (C_UTILIZZA_SEMPRE_DEFAULTS == "SI" or C_UTILIZZA_SEMPRE_DEFAULTS == "AUTO") {
            ...
            } # fine if (C_UTILIZZA_SEMPRE_DEFAULTS == "SI" or C_UTILIZZA_SEMPRE_DEFAULTS == "AUTO")
...
?>");
                            ...
                            if (C_CREADB_CITTA_DEFAULT == "SI") aggiorna_relutenti("", "SI", "", "", $id_utente, $id_utente, "", "", "", "", "", "", "", "citta", "citta", $tablecitta, $tablerelutenti, "regione", "regioni", $tableregioni);
                            ...
                            if (C_NASCONDI_MARCA == "SI" and @is_file(C_CARTELLA_CREA_MODELLI . "/index.html")) @unlink(C_CARTELLA_CREA_MODELLI . "/index.html");
...
// inserisco i dati forniti nella tabella appartamenti e creo il file selezione appartamenti.
        if ($insappartamenti and !@is_file(C_DATI_PATH . "/selectappartamenti.php")) {
            ...
            if (C_UTILIZZA_SEMPRE_DEFAULTS == "AUTO" and $num_app_max < 10) $num_app_max = 10;
            for ($num = 1; $num <= ($numappartamenti + $numletti); $num = $num + 1) {
                $numapp = "numapp" . $num;
                $numapp = $$numapp;
                if (C_UTILIZZA_SEMPRE_DEFAULTS == "SI" or C_UTILIZZA_SEMPRE_DEFAULTS == "AUTO") {
                    if ($num <= $numappartamenti) $numapp = (string)substr($zeri, 0, (strlen($num_app_max) - strlen($num))) . $num;
                        ...
                        $numapp = $num_dorm . substr($lettere, $pos_lettera, 1);
                        $pos_lettera++;
                        if ($pos_lettera == 26) $pos_lettera = 0;
                    } # fine else if ($num <= $numappartamenti)
                } # fine if (C_UTILIZZA_SEMPRE_DEFAULTS == "SI" or C_UTILIZZA_SEMPRE_DEFAULTS == "AUTO")
...
");
            ...
            if (C_NASCONDI_MARCA != "SI") $testo .= "<h4>" . mex("Benvenuto a HotelDruid!", $pag) . "</h4><br>";
...
<li>" . mex("Vai alla pagina", $pag) . "
 ...
 " . mex("per cambiare il nome della valuta, abilitare la registrazione delle entrate, inserire i metodi di pagamento, ed impostare molte altre opzioni", $pag) . ".<br><br></li>
            ...
            if (C_NASCONDI_MARCA == "SI") $testo = str_replace("HotelDruid", mex("questo programma", $pag), $testo);
