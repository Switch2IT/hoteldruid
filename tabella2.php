...
if ($id_utente) {
    ...
    if ($id_utente != 1) {
        ...
        if (numlin_query($privilegi_annuali_utente) == 0) $anno_utente_attivato = "NO";
            ...
            if ($priv_vedi_tab_mesi == "q" or $priv_vedi_tab_mesi == "r" or $priv_vedi_tab_mesi == "g") {
                $priv_vedi_tab_mesi = "p";
                $prendi_gruppi = "SI";
            }
...
<table style=\"margin-left: auto; margin-right: auto;\" border=1 cellspacing=0 cellpadding=1>";
            ...
            if ($mese2 != $mese_inizio_periodi) {
                ...
                if ($g_inizio_tab == 7 or $g_inizio_tab == 6) {
            ...
            if ($g_fine_tab == 8 or $g_fine_tab == 9 or $g_fine_tab == 10) {
                $id_data_fine_tab = $id_data_fine_tab - 1;
            }
            if ($mese2 != $mese_fine_periodi) {
                if ($g_fine_tab == 7 or $g_fine_tab == 6) {
                    $id_data_fine_tab = $id_data_fine_tab - 1;
                }
...
# controllo se vi sono prenotazioni in appartamenti cancellati o non mostrati
            ...
            if ($priv_vedi_tab_mesi == "p") {
                ...
                if ($priv_prenota_gruppi == "SI") {
                    reset($utenti_gruppi);
                    while (list($idut_gr, $val) = each($utenti_gruppi)) if ($idut_gr != $id_utente) $query_prenota_app_canc .= " or utente_inserimento = '$idut_gr'";
...
";
            for ($num1 = 0; $num1 < $num_colonne; $num1 = $num1 + 1) {
                ...
                for ($num2 = 0; $num2 < 8; $num2 = $num2 + 1) {
                    ...
                    if ($num_giorno < 1) {
                        $num_giorno = $g_inizio_settimana + $num2;
                    }
            ...
            for ($num1 = 0; $num1 < ($num_appartamenti + $num_app_canc); $num1 = $num1 + 1) {
                if ($num1 < $num_appartamenti) {
                    ...
                    if ($priv_vedi_tab_mesi == "p") {
                        ...
                        if ($priv_prenota_gruppi == "SI") {
                            reset($utenti_gruppi);
                            while (list($idut_gr, $val) = each($utenti_gruppi)) if ($idut_gr != $id_utente) $condizione_prenota_propria .= " or utente_inserimento = '$idut_gr'";
                        } # fine if ($priv_prenota_gruppi == "SI")
                        $condizione_prenota_propria .= " )";
                    } # fine if ($priv_vedi_tab_mesi == "p")
                        ...
                        if ($priv_vedi_tab_mesi == "p" and (($utente_inserimento != $id_utente and ($priv_prenota_gruppi != "SI" or !$utenti_gruppi[$utente_inserimento])) or ($periodo_consentito_app[$id_appartamento][$id_settimana] == "NO" and $esiste != 1))) {
                            ...
                            if ($esiste == 1) {
                                ...
                                if ($id_inizio_prenota > $id_data_inizio_tab) {
                                    $id_inizio = $id_inizio_prenota;
                                } else {
                                    $id_inizio = $id_data_inizio_tab;
                                }
                                if ($id_fine_prenota < $id_data_fine_tab) {
                                    $id_fine = $id_fine_prenota;
                                } else {
                                    $id_fine = $id_data_fine_tab;
                                }
                                $colonne = $id_fine - $id_inizio + 1;
