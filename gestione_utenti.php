...
if ($id_utente and $id_utente == 1) {
    ...
    if ($modifica_utenti) {
        ...
        if (C_RESTRIZIONI_DEMO_ADMIN == "SI") $cond_escludi_admin = " where idutenti != '1'";
...
<input type=\"hidden\" name=\"continua\" value=\"SI\">";
            for ($num1 = 0; $num1 < numlin_query($lista_utenti); $num1++) {
                ...
                if (C_DISABILITA_PASS_ADMIN == "NO" and $id == 1 and $tipo_pass != "n" and $n_tipo_pass == "n") $n_tipo_pass = $tipo_pass;
                if ($tipo_pass != $n_tipo_pass) {
                    $cambiato = "SI";
                    echo mex("Il <b>login</b> dell'utente", $pag) . " <b>$id</b> " . mex("verrà cambiato da", $pag) . " <i>";
                    switch ($tipo_pass) {
                        case "t":
                            echo mex("password conservata in chiaro", $pag);
                            break;
                        case "5":
                            echo mex("password conservata criptata con md5", $pag);
                            break;
                        case "c":
                            echo mex("password conservata criptata con mcrypt", $pag);
                            break;
                        case "h":
                            echo mex("password conservata criptata con mhash", $pag);
                            break;
                        default:
                            echo mex("disabilitato", $pag);
...
<button class=\"gobk\" type=\"submit\"><div>" . mex("Torna indietro", $pag) . "</div></button>
                ...
                esegui_query("insert into $tableprivilegi (idutente,anno,casse_consentite,priv_mod_pers,priv_ins_clienti,prefisso_clienti,priv_messaggi,priv_inventario) values ('$nuovo_id','1','n,','nnnn','nnnss','n,','nn','nnnnnnnnn')");
...
#aggiorna_relutenti("","","","SI",$id_utente,$nuovo_id,"","","","","",1,"","citta","citta",$tablecitta,$tablerelutenti);
    ...
    if ($modifica_pass) {
        ...
        if ($id_utente_pass == 1 and C_RESTRIZIONI_DEMO_ADMIN == "SI") $continua = "NO";
    ...
    if ($cancella) {
        ...
        if ($dati_collegati != "SI") {
            ...
            for ($num2 = 0; $num2 < $num_anni; $num2++) {
                ...
                if (numlin_query($prenota_esist) > 0) {
                    $dati_collegati = "SI";
                    break;
                }
...
<button class=\"gobk\" type=\"submit\"><div>" . mex("NO", $pag) . "</div></button>
        ...
        for ($num_imp = 0; $num_imp < count($id_utenti_importa); $num_imp++) {
            ...
            if ($id_utente_importa >= 2 and $id_utente_esporta >= 2 and $id_utente_importa != $id_utente_esporta) {
                ...
                if (numlin_query($utente_imp) == 1 and numlin_query($utente_esp) == 1) {
                    ...
                    if ($tipo_importa == "pers" or $tipo_importa == "priv_pers") {
                        ...
                        $pers_importate = "SI";
                    } # fine if ($tipo_importa == "pers" or $tipo_importa == "priv_pers")

                    if ($tipo_importa == "priv" or $tipo_importa == "priv_pers") {
 ...
 values ('$id_utente_importa','$e_anno','$e_regole1_consentite','$e_tariffe_consentite','$e_costi_agg_consentiti','$e_contratti_consentiti','$e_casse_consentite','$e_cassa_pagamenti','$e_priv_ins_prenota','$e_priv_mod_prenota','$e_priv_mod_pers','$e_priv_ins_clienti','$e_prefisso_clienti','$e_priv_ins_costi','$e_priv_vedi_tab','$e_priv_ins_tariffe','$e_priv_ins_regole','$e_priv_messaggi','$e_priv_inventario')");
                        ...
                        $priv_importati = "SI";
                    } # fine if ($tipo_importa == "priv" or $tipo_importa == "priv_pers")
...
<td align=\"center\" valign=\"middle\"><select name=\"tipo_pass$id\">";
            if (C_DISABILITA_PASS_ADMIN != "NO" or $id != 1 or $disab_sel) echo "<option value=\"n\"$disab_sel>" . mex("disabilitato", $pag) . "</option>";
