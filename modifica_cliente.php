...
<button class=\"gobk\" type=\"submit\"><div>" . mex("Torna indietro", $pag) . "</div></button>
            ...
            if ($modifica_cliente) {
                ...
                if ($modifica_cliente != "Continua") {
                    ...
                    if ($n_nome) {
                        echo mex("Il nome verrà cambiato da", $pag) . " <b>$d_nome</b> " . mex("a", $pag) . " <b>$n_nome</b>.<br>";
                    }
                    if ($n_soprannome) {
                        echo mex("Il soprannome verrà cambiato da", $pag) . " <b>$d_soprannome</b> " . mex("a", $pag) . " <b>$n_soprannome</b>.<br>";
                    }
                    if ($n_titolo_cli) {
                        echo mex("Il titolo verrà cambiato da", $pag) . " <b>$d_titolo_cli</b> " . mex("a", $pag) . " <b>$n_titolo_cli</b>.<br>";
                    }
                    if ($n_sesso) {
                        echo mex("Il sesso verrà cambiato a", $pag) . " <b>$n_sesso</b>.<br>";
                    }
                    ...
                    if ($n_nazionalita) {
                        echo mex("La cittadinanza verrà cambiata da", $pag) . " <b>$d_nazionalita</b> " . mex("a", $pag) . " <b>$n_nazionalita</b>.<br>";
                    }
                    if ($n_telefono) {
                        echo mex("Il telefono verrà cambiato da", $pag) . " <b>$d_telefono</b> " . mex("a", $pag) . " <b>$n_telefono</b>.<br>";
                    }
...
" . ucfirst(mex("password per la visualizzazione delle carte di credito", $pag)) . ":
 ...
 <button class=\"login\" type=\"submit\"><div>" . ucfirst(mex("invia", $pag)) . "</div></button>
                                    ...
                                    esegui_query("delete from $tabletransazioni where ultimo_accesso <= '$limite_transazioni_vecchie' and (tipo_transazione = 'mo_cc' or tipo_transazione = 'er_cc') ");
...
values ('$id_transazione','$id_sessione','er_cc','$anno','$idclienti','$id_utente','$ultimo_accesso')");
                                    unlock_tabelle($tabelle_lock);
                                    if (numlin_query($err_transazioni) < 5) {
                                        ...
                                        if ($val_decrypt == 'prova') {
                                            ...
                                            if (numlin_query($dati_carta)) {
                                                ...
                                                if ($d_cvc_cc) openssl_private_decrypt(base64_decode($d_cvc_cc), $d_cvc_cc, $res_pk);
...
<option value=\"\" selected>--</option>";
                                            ...
                                            if ($gest_cvc == "SI") {
                                                echo "<tr style=\"background-color: " . rowbgcolor() . ";\"><td>" . mex("CVC", $pag) . ": <b>$d_cvc_cc</b></td>
<td>" . mex("cambia in", $pag) . ": 
<input type=\"text\" name=\"n_cvc_cc\" size=\"6\"></td></tr>
...
<button class=\"canc\" type=\"submit\"><div>" . ucfirst(mex("cancella questa carta di credito", $pag)) . "</div></button>
                                    ...
                                    } # fine if (numlin_query($err_transazioni) < 5)
...
<td$class_opt2>" . mex("Commento", $pag) . "</td></tr>";
                        ...
                        for ($num2 = 0; $num2 < $num_prenotazioni; $num2 = $num2 + 1) {
                            ...
                            if ($priv_vedi_tab_prenotazioni == "s" or ($priv_vedi_tab_prenotazioni == "p" and $utente_inserimento_prenota == $id_utente) or ($priv_vedi_tab_prenotazioni == "g" and $utenti_gruppi[$utente_inserimento_prenota])) {
                                ...
                                if (!$num_persone or $num_persone == 0) {
                                    $num_persone = "?";
                                }
                                $n_letti_agg = 0;
