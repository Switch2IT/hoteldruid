...
if (!function_exists('gzopen') and function_exists('gzopen64')) {
    ...
    {
        return gzopen64($arg1, $arg2);
    }

    if (!function_exists('gzread') and function_exists('gzread64')) {
        function gzread(&$arg1, $arg2)
        {
            return gzread64($arg1, $arg2);
        }
    }
    if (!function_exists('gzwrite') and function_exists('gzwrite64')) {
        function gzwrite(&$arg1, $arg2)
        {
            return gzwrite64($arg1, $arg2);
        }
    }
    if (!function_exists('gzclose') and function_exists('gzclose64')) {
        function gzclose(&$arg1)
        {
            return gzclose64($arg1);
        }
    }
...
");
    if ($tabella == "transazioni") $contenuto_tab = esegui_query("select * from $PHPR_TAB_PRE$tabella where tipo_transazione != 'err_l'");
        ...
        if ($tabella == "contratti") $contenuto_tab = esegui_query("select * from $PHPR_TAB_PRE$tabella order by tipo desc, numero");
        else $contenuto_tab = esegui_query("select * from $PHPR_TAB_PRE$tabella $cond");
                    ...
                    if ($leggendo_database == "SI") {
                        ...
                        if ($leggendo_tabella == "SI") {
                            ...
                            if ($linea == "</colonnetabella>" and $leggendo_righe != "SI") {
                                ...
                                } else if (substr($nome_tabella, 0, 5) == "costi") {
                            ...
                            if ($nome_file) {
                                if ($leggendo_contenuto == "SI") {
                                    ...
                                    if ($nome_file == "./dati/versione.php") {
                                        if ($linea != "" and $linea != "<?php" and $linea != "define(C_VERSIONE_ATTUALE," . C_PHPR_VERSIONE_NUM . ");" and preg_replace("/define\(C_DIFF_ORE,-?[0-9]{1,2}\);/", "", $linea) != "" and $linea != "?>") {
                                            $file_coerente = "NO";
                                            $num_err .= "#34";
                                        }
                                    ...
                                    if ($nome_file == "./dati/abilita_login" and $linea != "") {
                                        $file_coerente = "NO";
                                        $num_err .= "#41";
                                    }
                                    if ($nome_file == "./dati/lingua.php" and $linea != "" and $linea != "<?php" and $linea != "?>" and preg_replace("/\\\$lingua\\[[0-9]{1,8}\\] = \"[a-z]{1,3}\";/", "", $linea) != "") {
                                        $file_coerente = "NO";
                                        $num_err .= "#42";
                                    }
                                    ...
                                    if ($nome_file == "./dati/tema.php" and $linea != "" and $linea != "<?php" and $linea != "?>" and preg_replace("/\\\$tema\\[[0-9]{1,8}\\] = \"[a-z]{1,4}\";/", "", $linea) != "" and preg_replace("/\\\$parole_sost = [01];/", "", $linea) != "") {
                                        $file_coerente = "NO";
                                        $num_err .= "#43";
                                    }
                                    if ($nome_file == "./dati/parole_sost.php" and $linea != "" and $linea != "<?php" and $linea != "?>" and preg_replace("/\\\$messaggio = str_replace\\(\"[^\"]*\",\"[^\"]*\",\\\$messaggio\\);/", "", $linea) != "") {
                                        $file_coerente = "NO";
                                        $num_err .= "#44";
                                    }
                                    if ($nome_file == "./dati/dati_interconnessioni.php") {
                                        ...
                                        if ($linea_trovata != "SI") {
                                            $file_coerente = "NO";
                                            $num_err .= "#45<br>" . htmlspecialchars($linea) . "<br>";
                                        }
                                    } # fine if ($nome_file == "./dati/dati_interconnessioni.php")
                                    if ($nome_file == "./dati/log_utenti.php" and ($prima_linea != "<?php exit(); ?>" or ($linea != "<?php exit(); ?>" and preg_replace("/^[0-9]+>[0-9]{4,4}-[0-9]{2,2}-[0-9]{2,2} [0-9]{2,2}:[0-9]{2,2}:[0-9]{2,2}>[a-zA-Z]?.*/", "", $linea) != ""))) {
                                        $file_coerente = "NO";
                                        $num_err .= "#46";
                                    }
