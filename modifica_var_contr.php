...
<input class=\"sbutton\" type=\"submit\" name=\"torna\" value=\"" . mex("Aggiungi", $pag) . "\">
            ...
            if ($aggiungi_cond) {
                $mostra_form_iniziale = "NO";
                $continua = "SI";
                if ($inizializza != "rpt" and $inizializza != "inr" and $inizializza != "ind") $continua = "NO";
                ...
                if ($continua == "SI") {
                    ...
                    if ($azione == "set") {
                        ...
                        if ($tipo_val_allora == "txt") $val_allora = controlla_testo_input($val_allora_txt);
                        ...
                        $testo .= "set#%?$var_allora#%?$oper_str#%?$tipo_val_allora#%?$val_allora#%?$tipo_val_sost1#%?$val_sost1#%?$tipo_val_sost2#%?$val_sost2";
                    } # fine if ($azione == "set")
                    if ($azione == "trunc") {
                        controlla_var_allora($var_trunc, $continua, $contr_cond);
                        if (!$val_trunc or controlla_num_pos($val_trunc) == "NO") $continua = "NO";
...
<input class=\"sbutton\" type=\"submit\" value=\"" . mex("Modifica", $pag) . "\">
            ...
            if ($option_var_pers) {
                ...
                $azione_orig = $azione;
                $num_cond_mod_vedi = "";
                $condizioni = esegui_query("select * from $tablecontratti where tipo = 'cond$contr_cond' order by testo $LIKE 'ind%' desc, testo $LIKE 'inr%' desc, numero");
                $num_condizioni = numlin_query($condizioni);
                $num_cond_ini_d = 0;
                $num_cond_ini_r = 0;
                for ($num1 = 0; $num1 < $num_condizioni; $num1++) {
                ...
                if ($num_condizioni > 0) {
                    ...
                    for ($num1 = 0; $num1 < $num_condizioni; $num1++) {
                        ...
                        if ($azione[0] == "set") {
                            ...
                            if ($azione[3] == "var") $str_cond .= " [" . mex2($azione[4]) . "]";
                        ...
                        if ($azione[0] == "trunc") {
                            $str_cond .= mex("troncare", $pag) . " [" . $nome_var[$azione[1]] . "] " . mex("dopo", $pag);
                            $str_cond .= " " . $azione[2] . " " . mex("caratteri", $pag);
                            if (strcmp($azione[3], "")) {
                        ...
                        if ($num1 != ($num_condizioni - 1) and $num1 != ($num_cond_ini_r - 1) and $num1 != ($num_cond_ini_d - 1)) {
                            $opt_giu = "";
...
<img style=\"display: block;\" src=\"./img/freccia_giu_marg.png\" alt=\"&lt;\">
                        ...
                        } # fine if ($num1 != ($num_condizioni - 1) and...
                        else $tab_cond .= "<td></td>";
                        if ($num1 != 0 and $num1 != $num_cond_ini_r and $num1 != $num_cond_ini_d) {
                            $opt_su = "";
                            $fine_opt = 1;
...
<img style=\"display: block;\" src=\"./img/freccia_su_marg.png\" alt=\"&gt;\">
                        ...
                        } # fine if ($num1 != 0 and ($num1 + 1) != $num_cond_ini)
                        else $tab_cond .= "<td></td>";
...
</select></td><td>" . str_replace(" ", "&nbsp;", mex("dopo", $pag)) . "</td><td>
