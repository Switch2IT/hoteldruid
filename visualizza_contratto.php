...
# I controlli di non vuoto servono a risparmiare memoria, creando solo le variabili necessarie
                                        ...
                                        if (strcmp(risul_query($dati_cliente, 0, 'cittanascita'), "")) ${"citta_nascita_" . $num1} = risul_query($dati_cliente, 0, 'cittanascita');
                                        if (strcmp(risul_query($dati_cliente, 0, 'regionenascita'), "")) ${"regione_nascita_" . $num1} = risul_query($dati_cliente, 0, 'regionenascita');
                                        if (strcmp(risul_query($dati_cliente, 0, 'nazionenascita'), "")) ${"nazione_nascita_" . $num1} = risul_query($dati_cliente, 0, 'nazionenascita');
                                        if (strcmp(risul_query($dati_cliente, 0, 'nazionalita'), "")) ${"cittadinanza_" . $num1} = risul_query($dati_cliente, 0, 'nazionalita');
                                        if (strcmp(risul_query($dati_cliente, 0, 'lingua'), "")) ${"codice_lingua_" . $num1} = risul_query($dati_cliente, 0, 'lingua');
                                        if (strcmp(risul_query($dati_cliente, 0, 'nazione'), "")) ${"nazione_" . $num1} = risul_query($dati_cliente, 0, 'nazione');
                                        if (strcmp(risul_query($dati_cliente, 0, 'regione'), "")) ${"regione_" . $num1} = risul_query($dati_cliente, 0, 'regione');
                                        if (strcmp(risul_query($dati_cliente, 0, 'citta'), "")) ${"citta_" . $num1} = risul_query($dati_cliente, 0, 'citta');
                                        if ($priv_vedi_indirizzo == "s") {
                                            if (strcmp(risul_query($dati_cliente, 0, 'via'), "")) ${"via_" . $num1} = risul_query($dati_cliente, 0, 'via');
                                            if (strcmp(risul_query($dati_cliente, 0, 'numcivico'), "")) ${"numcivico_" . $num1} = risul_query($dati_cliente, 0, 'numcivico');
                                            if (strcmp(risul_query($dati_cliente, 0, 'cap'), "")) ${"cap_" . $num1} = risul_query($dati_cliente, 0, 'cap');
                                        } # fine if ($priv_vedi_indirizzo == "s")
                                        if (strcmp(risul_query($dati_cliente, 0, 'documento'), "")) ${"documento_" . $num1} = risul_query($dati_cliente, 0, 'documento');
                                        if (strcmp(risul_query($dati_cliente, 0, 'tipodoc'), "")) ${"tipo_documento_" . $num1} = risul_query($dati_cliente, 0, 'tipodoc');
                                        if (strcmp(risul_query($dati_cliente, 0, 'cittadoc'), "")) ${"citta_documento_" . $num1} = risul_query($dati_cliente, 0, 'cittadoc');
                                        if (strcmp(risul_query($dati_cliente, 0, 'regionedoc'), "")) ${"regione_documento_" . $num1} = risul_query($dati_cliente, 0, 'regionedoc');
                                        if (strcmp(risul_query($dati_cliente, 0, 'nazionedoc'), "")) ${"nazione_documento_" . $num1} = risul_query($dati_cliente, 0, 'nazionedoc');
                                        if (strcmp(risul_query($dati_cliente, 0, 'scadenzadoc'), "")) ${"scadenza_documento_" . $num1} = risul_query($dati_cliente, 0, 'scadenzadoc');
                                        if ($priv_vedi_telefoni == "s") {
                                            if (strcmp(risul_query($dati_cliente, 0, 'telefono'), "")) ${"telefono_" . $num1} = risul_query($dati_cliente, 0, 'telefono');
                                            if (strcmp(risul_query($dati_cliente, 0, 'telefono2'), "")) ${"telefono2_" . $num1} = risul_query($dati_cliente, 0, 'telefono2');
                                            if (strcmp(risul_query($dati_cliente, 0, 'telefono3'), "")) ${"telefono3_" . $num1} = risul_query($dati_cliente, 0, 'telefono3');
                                            if (strcmp(risul_query($dati_cliente, 0, 'fax'), "")) ${"fax_" . $num1} = risul_query($dati_cliente, 0, 'fax');
                                            if (strcmp(risul_query($dati_cliente, 0, 'email'), "")) ${"email_" . $num1} = risul_query($dati_cliente, 0, 'email');
                                        } # fine if ($priv_vedi_telefoni == "s")
                                        if (strcmp(risul_query($dati_cliente, 0, 'cod_fiscale'), "")) ${"codice_fiscale_" . $num1} = risul_query($dati_cliente, 0, 'cod_fiscale');
                                        if (strcmp(risul_query($dati_cliente, 0, 'partita_iva'), "")) ${"partita_iva_" . $num1} = risul_query($dati_cliente, 0, 'partita_iva');
                                        ...
                                        for ($num2 = 0; $num2 < $num_dati_relcliente; $num2++) {
                                            ${"campo_personalizzato_" . risul_query($dati_relcliente, $num2, 'testo1') . "_" . $num1} = risul_query($dati_relcliente, $num2, 'testo3');
                                        } # fine for $num2
                                ...
                                if ($id_data_inizio) {
                                    ...
                                    if (strcmp($sconto, "")) ${"sconto_" . $num1} = $sconto;
                                    if (strcmp(risul_query($dati_prenota, 0, 'tasseperc'), "")) ${"percentuale_tasse_tariffa_" . $num1} = risul_query($dati_prenota, 0, 'tasseperc');
                                    if (strcmp(risul_query($dati_prenota, 0, 'commento'), "")) ${"commento_" . $num1} = risul_query($dati_prenota, 0, 'commento');
                                    if (strstr(${"commento_" . $num1}, ">")) {
                                        $comm = explode(">", ${"commento_" . $num1});
                                        ${"commento_" . $num1} = $comm[0];
                                        if (strcmp($comm[1], "")) ${"promemoria_entrata_" . $num1} = $comm[1];
                                        if (strcmp($comm[2], "")) ${"promemoria_uscita_" . $num1} = $comm[2];
                                        for ($num2 = 3; $num2 < count($comm); $num2++) {
                                            $comm_pers = explode("<", $comm[$num2]);
                                            if (strcmp($comm_pers[1], "")) ${"commento_personalizzato_" . $comm_pers[0] . "_" . $num1} = $comm_pers[1];
                                        } # fine for $num2
                                    ...
                                    for ($numca = 0; $numca < $dati_cap['num']; $numca++) {
                                        ...
                                        if ($dati_cap[$numca]['associasett'] == "s") {
                                            if ($dati_cap[$numca]['settimane']) ${"giorni_costo_agg" . $numca . "_" . $num1} = $dati_cap[$numca]['settimane'];
                                    ...
                                    if ($priv_vedi_tab_costi != "n") {
                                            ...
                                            for ($num2 = 1; $num2 < (count($id_anni_prec_vett) - 1); $num2++) {
                                                ...
                                                if (numlin_query($anno_prec_esistente)) {
                                                    ...
                                                    for ($num3 = 0; $num3 < $num_pagamenti2; $num3++) {
                                                        ...
                                                        if (strcmp(risul_query($pagamenti, $num3, 'metodo_pagamento'), "")) ${"metodo_paga" . $num_pagamenti . "_" . $num1} = risul_query($pagamenti, $num3, 'metodo_pagamento');
                                        ...
                                        for ($num2 = 0; $num2 < $num_pagamenti2; $num2++) {
                                            ...
                                            if (strcmp(risul_query($pagamenti, $num2, 'metodo_pagamento'), "")) ${"metodo_paga" . $num_pagamenti . "_" . $num1} = risul_query($pagamenti, $num2, 'metodo_pagamento');
