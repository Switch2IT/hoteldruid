                                    ...
                                    for ($num2 = 0; $num2 < $num_trova_email_tm; $num2++) {
                                        if (!$mittente_email_tm[$num2] or stristr($mittente, $mittente_email_tm[$num2])) {
                                            if (!strcmp($oggetto_email_tm[$num2], "") or stristr($oggetto, $oggetto_email_tm[$num2])) {
                                                if ($therad_email_tm[$num2] == "tutti" or (!$risposta_a and !$riferimento and substr($oggetto, 0, 4) != "Re: ")) {
                                                    $inserisci_mess = 1;
                                                    $mess_scaricati++;
                                                    break;
...
#echo "<br><br>".var_dump($eml_header)."<br><br>";
                                        ...
                                        if ((stristr($testo, "Content-Type: multipart/alternative;") or stristr($testo, "This is a multi-part message in MIME format")) and stristr($testo, "Content-Type: text/plain;")) {
...
# strano syntax error se si rimuove "." (dovuto a riconoscimento array?)
                                                ...
                                                if (!$arrivo[1]) $arrivo = preg_split("/$fr_date" . "[^0-9a-z]/i", $testo);
                                                if ($arrivo[1]) {
                                                    $arrivo = preg_split("/$fr_partenza" . "[^0-9a-z]/i", substr($testo, strlen($arrivo[0])));
                                                    $arrivo = trova_prima_data(substr($arrivo[0], 0, 40), $stile_data, $lingue[$num2]);
                                            ...
                                            if (!$partenza_trovata and !$partenza_cercata[$fr_partenza]) {
                                                ...
                                                if ($partenza[1]) {
                                                    $partenza = trova_prima_data(substr($testo, strlen($partenza[0]), 40), $stile_data, $lingue[$num2]);
                                        ...
                                        if ($oracheckin) {
                                            ...
                                            if (controlla_num_pos($giorno_ini) == "SI") {
                                                ...
                                                for ($num2 = $giorno_ini; $num2 <= $giorno_fine; $num2++) {
                                                    ...
                                                    if ($data_select == $data_checkin) {
                                                        $giorno_stima_checkin = $num2 - $giorno_ini + 1;
                                                        break;
                                                    }
                                                } # fine for $num2
...
<button class=\"canc\" type=\"submit\"><div>" . mex("Elimina", $pag) . "</div></button>
                ...
                echo "</table>$stringa_pagine<br><div style=\"text-align: center\">
